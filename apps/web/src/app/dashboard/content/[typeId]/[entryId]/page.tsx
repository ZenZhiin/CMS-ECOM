            ...
            <div className={styles.fieldGrid}>
              {contentType.fields.map((field: any) => (
                <div key={field.name} className={styles.fieldItem}>
                  ...
                  {field.type === 'text' && <input type="text" className={styles.input} value={formData[field.name] || ''} onChange={(e) => handleChange(field.name, e.target.value)} />}
                  {field.type === 'number' && <input type="number" className={styles.input} value={formData[field.name] || 0} onChange={(e) => handleChange(field.name, Number(e.target.value))} />}
                  {field.type === 'boolean' && (
                  ...
                  {field.type === 'date' && <input type="date" className={styles.input} value={formData[field.name] || ''} onChange={(e) => handleChange(field.name, e.target.value)} />}
                </div>
              ))}
            </div>
          </div>
